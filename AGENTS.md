...
- **Engine notes**: Win32 threads on Windows (DLL stays self-contained under MinGW, no pthread link), pthreads on Linux (CMake `find_package(Threads)`). An AVX2 path (`crack_mt8_block`) scores **8 seeds per call** (the full `__m256i` width — 4 lanes sat idle before; widening was a measured 2.23× on the sweep), runtime-dispatched via `__builtin_cpu_supports("avx2")` with a scalar fallback. Targets are sorted fewest-candidate-regions-first (strongest filter early — 1.9× on the sweep once the AVX2 lanes were saturated). Validation errors are returned as `{"error": ...}` JSON. `core/CMakeLists.txt` forces `CMAKE_BUILD_TYPE=Release` when unset: an empty build type left the DLL at `-O0`, ~6× slower on the crack (2.8M vs 17M seeds/s).
...
`wasm/CMakeLists.txt` compiles `core/seedfinder_wrapper.c` + `core/crack64.c` + `core/crack_mt.c` + `ChunkBiomesGUI/Bfinders.c` + cubiomes sources **directly** (no DLL link) with Emscripten into `wasm/out/seedcracker.{wasm,js}` (ES6 modularized glue). The `core/crack_mt.c` entry is **mandatory** (added 2026-09-17): without it the link fails on `sweepMtSurvivors`/`crackScore8`. Design docs: `docs/superpowers/{specs,plans}/2026-09-05-wasm-seedcracker*.md`; **`wasm/INTEGRATION.md` is the self-contained reference** for consuming the module (ABI table, worker scheduler, memory discipline) — read it before touching the wasm side. Both `wasm/` and `docs/` are gitignored (exception: `docs/verification/bedrock64-empirical.md` is tracked) — so `wasm/CMakeLists.txt` lives only on this machine and a fresh clone must re-add that line before building WASM. Build via `wasm/build_wasm.bat`; if its `cmake --build` step silently no-ops, run `cmake --build wasm/build -j N` manually with `C:\msys64\ucrt64\lib\emscripten` on PATH (that is where `emcc.bat`/`emcmake.bat` live; the `.exe` variants are `.broken`).
