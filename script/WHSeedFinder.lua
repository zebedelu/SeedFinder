-- ============================================================================
-- SeedFinder — Minecraft Bedrock Structure Finder (Hosted API Edition)
-- Uses network.get() to call the hosted SeedFinder API at mineseedfinder.vercel.app
-- ============================================================================
...
function onLoad()
	log("SeedFinder loaded (Hosted API Edition)")
	log("Using the hosted API at " .. SERVER_URL .. " — no local server needed.")
end
...
function onEnable()
	...
	if not checkServer() then
		if not serverWarned then
			serverWarned = true
			log("SeedFinder: Server offline at " .. SERVER_URL .. ". Check your internet connection and try again.")
		end
		return
	end
end
